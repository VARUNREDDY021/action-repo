...
module "critical_cpu_consumedthreshold" {
  source = "../cloudwatch"
  alarm_name          = "${var.project}-rds-${var.shortend_region}-critical-alert-cpu-consumed-high"
...
module "critical_memory_consumedthreshold" {
  source = "../cloudwatch"
  alarm_name          = "${var.project}-rds-${var.shortend_region}-critical-alert-memory-consumed-high"
...
module "db_connections" {
  source = "../cloudwatch"
  alarm_name          = "${var.project}-rds-${var.shortend_region}-critical-alert-db-connections"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  ...
  actions_enabled     = var.actions_enabled
  alarm_actions       = var.alarm_actions
  dimensions          = {DBClusterIdentifier = "DatabaseConnections"}
